...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full pt-3 sm:pt-4 px-3 sm:px-5">
      <div
        className="mx-auto max-w-7xl rounded-2xl sm:rounded-full transition-all duration-300"
        style={{
          background: scrolled
            ? "oklch(0.05 0.008 250 / 0.82)"
            : "oklch(0.06 0.01 250 / 0.55)",
          backdropFilter: "blur(22px) saturate(160%)",
          WebkitBackdropFilter: "blur(22px) saturate(160%)",
          boxShadow:
            "inset 0 1px 0 oklch(0.85 0.12 240 / 0.18), 0 0 0 1px oklch(0.7 0.22 240 / 0.35), 0 0 32px -8px oklch(0.5 0.22 250 / 0.55), 0 18px 50px -18px oklch(0 0 0 / 0.85)",
        }}
      >
        <div className="flex items-center justify-between gap-3 px-3 sm:px-5 py-2 sm:py-2.5">
          <Link to="/" className="flex items-center gap-2 shrink-0 group" aria-label="Sperin Services — Home">
          ...
          <div className="hidden md:flex items-center shrink-0">
            <a
              ...
              style={{
                ...
                boxShadow:
                  "inset 0 1px 0 oklch(0.85 0.12 240 / 0.2), 0 0 0 1px oklch(0.7 0.22 240 / 0.5), 0 0 22px -4px oklch(0.6 0.22 245 / 0.55)",
              }}
            >
              <Phone className="h-4 w-4 text-electric" style={{ filter: "drop-shadow(0 0 6px oklch(0.7 0.22 240 / 0.8))" }} />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 90% 60% at 50% 0%, oklch(0.12 0.05 250 / 0.95), oklch(0.04 0.005 250 / 0.98) 70%)",
          ...
          <nav className="flex flex-col gap-1 max-w-md mx-auto">
            {NAV.map((n, i) => (
              <Link
                ...
                className={`block rounded-2xl px-5 py-4 text-2xl font-semibold text-foreground/95 transition-all duration-500 ${
            ...
            <div className="mt-6 mb-2 px-2 text-xs font-semibold uppercase tracking-[0.2em] text-electric">
              Services
            </div>
            {SERVICES.map((s, i) => (
              <Link
                ...
                to={s.path}
                onClick={() => setOpen(false)}
                className={`block rounded-xl px-5 py-3 text-base text-foreground/85 hover:text-electric transition-all duration-500 ${
                  open ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
                }`}
                style={{
...
function NavItem({ to, label, exact }: { to: string; label: string; exact?: boolean }) {
  return (
    <Link
      ...
      className="group relative px-4 py-2 text-sm font-medium text-foreground/80 hover:text-foreground transition-colors"
      activeProps={{ className: "relative px-4 py-2 text-sm font-semibold text-electric" }}
    >
      <span className="relative z-10">{label}</span>
      <span
        aria-hidden
        className="pointer-events-none absolute left-1/2 -translate-x-1/2 bottom-1 h-px w-0 group-hover:w-8 transition-all duration-300"
        style={{
          background: "linear-gradient(90deg, transparent, oklch(0.72 0.22 240), transparent)",
          boxShadow: "0 0 10px oklch(0.7 0.22 240 / 0.9)",
        }}
      />
    </Link>
  );
...
function ServicesDropdown() {
  ...
  return (
    ...
    >
      <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-foreground/80 hover:text-electric transition-colors">
        Services <ChevronDown className={`h-4 w-4 transition-transform ${hover ? "rotate-180" : ""}`} />
      </button>
      {hover && (
          ...
          >
            {SERVICES.map((s) => (
              <Link
                key={s.slug}
                to={s.path}
                className="block rounded-xl px-3 py-2.5 text-sm text-foreground/85 hover:bg-electric/10 hover:text-electric transition"
              >
