import { SITE } from "@/lib/site";

export function WhatsAppGlyph({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" className={className} aria-hidden="true">
      <rect width="32" height="32" rx="7" fill="#25D366" />
      <path
        fill="#fff"
        transform="translate(4 4)"
        d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z"
      />
    </svg>
  );
}

export function WhatsAppButton({
  label = "Chat on WhatsApp",
  size = "md",
  className = "",
  iconOnly = false,
}: {
  label?: string;
  size?: "sm" | "md" | "lg";
  className?: string;
  iconOnly?: boolean;
}) {
  const sizeCls =
    size === "lg"
      ? "px-7 py-3.5 text-base"
      : size === "sm"
      ? "px-4 py-2 text-xs"
      : "px-6 py-3 text-sm";
  return (
    <a
      href={`https://wa.me/${SITE.whatsapp}`}
      target="_blank"
      rel="noreferrer"
      aria-label={iconOnly ? "Chat on WhatsApp" : undefined}
      className={`group inline-flex items-center justify-center gap-2.5 font-semibold text-foreground transition-all duration-300 hover:scale-[1.04] active:scale-95 ${
        iconOnly ? "h-16 w-16 rounded-[14px] p-0" : `rounded-full ${sizeCls}`
      } ${className}`}
      style={{
        background: iconOnly ? "transparent" : "oklch(0.08 0.012 250 / 0.7)",
        backdropFilter: iconOnly ? "none" : "blur(14px) saturate(160%)",
        WebkitBackdropFilter: iconOnly ? "none" : "blur(14px) saturate(160%)",
        boxShadow: iconOnly
          ? "0 8px 24px -8px rgba(0, 0, 0, 0.55), 0 0 0 1px rgba(255, 255, 255, 0.16)"
          : "inset 0 1px 0 oklch(0.95 0.1 150 / 0.15), 0 0 0 1px oklch(0.6 0.18 150 / 0.55), 0 0 22px -4px oklch(0.55 0.18 150 / 0.55), 0 8px 24px -10px oklch(0 0 0 / 0.7)",
      }}
    >
      <WhatsAppGlyph
        className={iconOnly ? "h-16 w-16" : size === "lg" ? "h-5 w-5" : "h-4 w-4"}
      />
      {!iconOnly && <span className="tracking-wide">{label}</span>}
    </a>
  );
}
